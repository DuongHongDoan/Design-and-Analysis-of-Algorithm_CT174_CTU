...
DoVat *ReadFromFile(int *W, int *n){
	 ...
	 int i=0;
 	 while (!feof(f)){
	   fscanf(f, "%d%d%d%[^\n]",&dsdv[i].TL,&dsdv[i].GT, &dsdv[i].SLMax, &dsdv[i].TenDV);
	   dsdv[i].PA=0;
	   i++;
...
void InDSDV(DoVat *dsdv ,int n, int W){
	...
	printf("\nPhuong an Cai Ba lo 1 dung thuat toan QUY HOACH DONG nhu sau:\n");
		printf("|---|------------------|----------|---------|--------------|-----------|\n");
		printf("|STT|     Ten Do Vat   | T Luong  | Gia Tri | So Luong Max | Phuong an |\n");
		printf("|---|------------------|----------|---------|--------------|-----------|\n");
	for(i=0;i<n;i++){
		printf("|%2d |%-18s|%5d     |%5d    |%6d        |%6d     |\n",
		i+1,dsdv[i].TenDV,dsdv[i].TL,dsdv[i].GT, dsdv[i].SLMax,dsdv[i].PA);
...
void TraBang(DoVat *dsdv, int n, int W, bang X) {
	...
	V = W;
	for(k= n-1; k>=0; k--) {
	 	dsdv[k].PA = min(dsdv[k].SLMax, X[k][V]);
